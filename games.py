...
def alpha_beta(game, state):
    ...
    best_score = -np.inf
    for a in game.actions(state):
        value = min_value(game.result(state, a), alpha, beta)
        if value > best_score:
            best_score = value
            best_action = a
...
def alpha_beta_cutoff(game, state):
    ...
    best_score = -np.inf
    for a in game.actions(state):
        value = min_value(game.result(state, a), alpha, beta, 0)
        if value > best_score:
            best_score = value
            best_action = a
        alpha = max(alpha, best_score)
...
def alpha_beta_player(game, state):
    ...
    if game.timer > 0 and time.perf_counter() < game.timer / 2:
...
def minmax_player(game, state):
    ...
    #print("Your code goes here -3pt")
    if len(state.board) <= 1:
        return random.choice(game.actions(state)) # use random move at start of speedup
    """Use a method to speed up at the start to avoid search down a long tree with not much outcome.
    Hint:for speedup use random_player for start of the game when you see search time is too long"""
    if (game.timer < 0):
        game.d = -1
...
class TicTacToe(Game):
    ...
    def eval1(self, state):
        ...
        """
        # player = self.to_move(state) # determine current player
        ...
        # iterate through all moves on board
