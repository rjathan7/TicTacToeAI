...
class MCTS: #Monte Carlo Tree Search implementation
    ...
    def monteCarloPlayer(self, timelimit = 4):
        ...
        #while count >= 0:
            ...
            selected_node = self.selectNode(self.root)

            # EXPAND stage
            #print("Your code goes here -2pt")
            if not self.isTerminalState(selected_node.state.utility, selected_node.state.moves):
                self.expandNode(selected_node)

            # SIMULATE stage using simuplateRandomPlay()
            #print("Your code goes here -3pt")
            node_to_simulate = selected_node
            if selected_node.children:
                node_to_simulate = random.choice(selected_node.children)
            winner = self.simulateRandomPlay(node_to_simulate)

            # BACKUP stage using backPropagation
            #print("Your code goes here -2pt")
            self.backPropagation(node_to_simulate, winner)
    ...
    def findBestNodeWithUCT(self, nd):
        ...
        childUCT = []
        #print("Your code goes here -2pt")
        best_child = max(nd.children, key=lambda child: self.uctValue(nd.visitCount, child.winScore, child.visitCount))
        return best_child
    ...
    def simulateRandomPlay(self, nd):
        ...
        while not self.isTerminalState(tempState.utility, tempState.moves):
            ...
            utility = self.game.compute_utility(tempState.board, action, tempState.board[action])
            tempState = tempState._replace(utility=utility)  # create a new GameState with updated utility
